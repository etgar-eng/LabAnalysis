# At the very beginning of main.py - FIRST LINES
import os
import warnings
...
IMG_WIDTH = 224
BATCH_SIZE = 32
NUM_CLASSES = 6  # 4 different lab logos
...
def predict_logo(model, image, class_names):
    ...
    if hasattr(image, 'resize'):
        ...
        # It's a PDFImageInfo object (from PyMuPDF/fitz)
        img = Image.open(io.BytesIO(image.image))
    elif hasattr(image, 'tobytes'):
        # It's some other image object with tobytes method
        img = Image.frombytes(image.mode, image.size, image.tobytes())
    else:
        # Try to convert whatever it is to PIL Image
        try:
