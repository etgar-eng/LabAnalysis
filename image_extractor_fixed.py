...
class PDFImageInfo:
    ...
    def save_image(self, filename: str = None):
        ...
        if not self.image_data or len(self.image_data) == 0:
            print(f"Error saving image: No image data available.")
            print(f"Solution: Use extract_images_hybrid_method() instead of extract_images_with_page_numbers().")
            return None
        ...
        try:
            ...
            print(f"Image format: {self.image_format}, Data size: {len(self.image_data)} bytes")
            return None
    ...
    def get_pil_image(self):
        ...
        # Check if image_data is empty
        if not self.image_data or len(self.image_data) == 0:
            print(f"Error creating PIL image: No image data available.")
            print(f"Solution: Use extract_images_hybrid_method() instead of extract_images_with_page_numbers().")
            return None
...
def extract_images_with_page_numbers(pdf_path: str, min_width: int = 50, min_height: int = 50) -> List[PDFImageInfo]:
    """
    Extract image metadata from PDF using pdfplumber (NO actual image data)
    
    WARNING: This function only extracts metadata, NOT actual image data!
    For saving/displaying images, use extract_images_hybrid_method() instead.
    ...
    Returns:
        List of PDFImageInfo objects containing image metadata (but empty image_data)
    """
    image_list = []
                ...
                if hasattr(page, 'images') and page.images:
                    for img_index, img_obj in enumerate(page.images):
                        ...
                        if width >= min_width and height >= min_height:
                            try:
                                # Create image info with EMPTY image_data
                                image_info = PDFImageInfo(
                                    image_data=b"",  # Empty - pdfplumber doesn't provide actual bytes
...
def extract_images_hybrid_method(pdf_path: str, min_width: int = 50, min_height: int = 50) -> List[PDFImageInfo]:
    ...
    This is the RECOMMENDED method for extracting images with actual data.
    
    Args:
        pdf_path: Path to the PDF file
        min_width: Minimum width to consider an image
        min_height: Minimum height to consider an image
        
    Returns:
        List of PDFImageInfo objects with actual image data
    """
...
def get_image_locations(pdf_path: str) -> List[Dict[str, Any]]:
    """
    Simple function to get image locations and page numbers
            ...
            if hasattr(page, 'images') and page.images:
                for img_index, img_obj in enumerate(page.images):
                    image_info = {
                        ...
                        'x1': img_obj.get('x1', 0),
                        'bottom': img_obj.get('bottom', 0),
                        'width': img_obj.get('x1', 0) - img_obj.get('x0', 0),
                        'height': img_obj.get('bottom', 0) - img_obj.get('top', 0),
                        'object_type': img_obj.get('object_type', 'unknown')
                    }
